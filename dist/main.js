...
$templateCache.put("views/rank-list.directive.html","<ul ng-if=\"!vm.locked\" class=\"top-selection\"><li ng-repeat=\"rank in vm.ranks track by $index\"><div ng-class=\"{ \'has-avatar\': rank.avatarUrl }\" ondragenter=\"return false\" ondragover=\"return false\" on-drop=\"vm.drop.handle(event, rank.value)\" class=\"shell\"><div ng-if=\"rank.id\" data-id=\"{{ rank.id }}\" draggable=\"draggable\"><avatar avatar-url=\"{{ rank.avatarUrl }}\"></avatar><div class=\"rank\">{{ rank.value }}</div></div><div ng-if=\"!rank.avatarUrl\" class=\"rank\">{{ rank.value }}</div></div></li><li ng-show=\"vm.confirm\"><button ng-click=\"vm.confirmRanks()\" class=\"action\">Confirm your selections</button></li></ul><p ng-show=\"vm.confirm\">Please confirm your winner selection. You will not be able to change this once you confirm.</p><ul ng-if=\"vm.locked\" class=\"top-selection\"><li ng-repeat=\"rank in vm.ranks track by $index\"><div ng-class=\"{ \'has-avatar\': rank.avatarUrl }\" class=\"shell\"><div ng-if=\"rank.id\" data-id=\"{{ rank.id }}\"><avatar avatar-url=\"{{ rank.avatarUrl }}\"></avatar><div class=\"rank\">{{ rank.value }}</div></div><div ng-if=\"!rank.avatarUrl\" class=\"rank\">{{ rank.value }}</div></div></li></ul>");
...
(function() {
  ...
  RankListController = function($scope, StepsService, RankListService, DataService) {
    var activate, projectId, render, stepId, userType, vm;
    vm = this;
    ...
    vm.confirmRanks = function() {
      return StepsService.confirmRanks(projectId, stepId);
    };
    vm.drop = {
      handle: function(event, rankToAssign) {
        ...
        if (submissionId !== 'undefined' && submissionId && rankToAssign) {
          return StepsService.updateRank(projectId, stepId, submissionId, rankToAssign);
        }
      }
    };
    activate();
